...
\section{Existing Risk-Annotated Scene Graph Framework}
...
\subsection{FastAPI-Based Interface}

{\small

In the baseline framework, FastAPI is used as the external communication interface between client applications and the internal scene graph generation pipeline. The FastAPI component is implemented through the API worker, which receives incoming requests, performs basic validation and conversion of the submitted data, and forwards valid RGB-D samples to the internal processing queue. In this design, the API layer is separated from the computationally expensive perception and scene graph generation modules.

The most important endpoint is the frame submission endpoint, which receives an RGB image, a depth image, timestamp information, and camera pose information. The API worker decodes the submitted RGB-D data, converts the depth image into a suitable numerical representation, validates the pose format, assigns a unique frame identifier, and stores the frame state as queued. The prepared frame is then inserted into the shared queue for later processing by the queue worker. As a result, FastAPI does not directly execute segmentation, retrieval, VLM inference, or scene graph merging. Instead, it acts as an asynchronous entry point that allows external clients to submit observations without being tightly coupled to the internal processing pipeline.

In addition to frame submission, the FastAPI interface provides endpoints for monitoring and retrieving system state. These include endpoints for checking whether the server is running, retrieving the latest scene graph, submitting robot-related information, querying the current robot node, checking the queue status, and tracking the processing state of individual frames. This makes the API layer responsible not only for accepting input data, but also for exposing the current state of the framework to external clients.

The FastAPI application is started using Uvicorn, which runs the configured application on the selected host and port. This allows the scene graph framework to operate as a network-accessible service. The use of a REST-based interface makes the original implementation relatively easy to test and connect to external programs, while keeping the perception and reasoning modules independent from the input source.
